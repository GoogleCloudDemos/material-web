...
export const styles = css`
  :host {
    ...
    outline: none;
    padding-block: calc(
      (var(--_container-height) - max(var(--_label-text-line-height), var(--_icon-size))) / 2
    );
  ...
  .button {
    ...
    min-width: calc(
      calc(64 * var(--md-sys-globalscale, 1px)) - var(--_leading-space) - var(--_trailing-space)
    );
    width: 100%;
    z-index: 0;
    height: 100%;
  ...
  @media (forced-colors: active) {
    .background {
      border: calc(1 * var(--md-sys-globalscale, 1px)) solid CanvasText;
    }
    :host(:is([disabled], [soft-disabled])) {
  ...
  :host([touch-target='wrapper']) {
    margin: max(
        ...
        (calc(48 * var(--md-sys-globalscale, 1px)) - var(--_container-height))/2
      )
      0;
  }
